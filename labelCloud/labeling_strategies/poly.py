...
class PolyStrategy(BaseLabelingStrategy):
    POINTS_NEEDED = 0
    PREVIEW = True
    ...
    def __init__(self, view: "GUI") -> None:
        super().__init__(view)
        #logging.info("Enabled poly mode.")
        ...
        self.points : Optional[list] = [] # polygon points
        ...
        #self.tmps : Optional[list] = [] # tmp polygon points for preview
        self.tmps : Optional[Point3D] = None # tmp polygon points for preview
        
        
        self.p1_w: Optional[Point3D] = None  # p1 + dir_vector
        self.p2_w: Optional[Point3D] = None  # p2 + dir_vector
        
        self.dir_vector: Optional[Point3D] = (0,0,0)  # p1 + dir_vector
    ...
    def draw_preview(self) -> None:
        ...
        #len_tmps = len(self.tmps)
        #logging.info(f"len_tmps {len_tmps}!")
        #logging.info(f"len_tmps {len_tmps}!")
        
        
        if len_points == 1:
            ogl.draw_points([self.points[0]], color=self.preview_color)
        elif len_points > 0 : #elif len_points > 0 : #and len_tmps > 0:
            ...
            for i in range(len_points-1):
                ogl.draw_points([self.points[i]], color=(1, 1, 0, 1))
                ogl.draw_lines([self.points[i], self.points[i+1]], color=self.preview_color)
            #ogl.draw_lines([self.points[len_points-1], tmps], color=self.preview_color)
            
            
    def get_bbox(self) -> BBox:
        assert self.points[0] is not None and self.points[1] is not None
        length = math3d.vector_length(np.subtract(self.points[0], self.points[1]))
