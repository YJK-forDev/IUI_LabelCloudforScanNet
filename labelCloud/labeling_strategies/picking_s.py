...
class PickingStrategy_s(BaseLabelingStrategy):
    POINTS_NEEDED = 1
    PREVIEW = True
        ...
        final_bbox = BBox(
            *np.add(
                ...
                [
                    ...
                    0.1,
                    0.1
                    #config.getfloat("LABEL", "STD_BOUNDINGBOX_WIDTH") / 2, #std_boundingbox_width = 0.55
                    #-config.getfloat("LABEL", "STD_BOUNDINGBOX_HEIGHT") / 3, #std_boundingbox_height = 0.15
                ],
            )
        )
